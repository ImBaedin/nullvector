...
const PLANET_COMPOSITIONS = ["metallic", "silicate", "icy", "volatileRich"] as const;

const STAR_KINDS = ["yellow_dwarf", "red_dwarf", "orange_dwarf", "blue_white"] as const;
...
export function generatePlanetsForSystem(args: {
	...
	for (let planetIndex = 0; planetIndex < planetCount; planetIndex += 1) {
		...
		const orbitalDistance = roundTo(0.55 + planetIndex * 0.45 + prng.nextInRange(0.0, 0.25), 4);
