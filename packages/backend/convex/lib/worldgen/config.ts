...
export function normalizeGenerationConfig(
	...
	const normalized = {
		...
		minCoreSectors: config.minCoreSectors ?? DEFAULT_GENERATION_CONFIG.minCoreSectors,
		...
		maxSectorsPerRun: config.maxSectorsPerRun ?? DEFAULT_GENERATION_CONFIG.maxSectorsPerRun,
	} satisfies NormalizedGenerationConfig;

	assertInteger("generationConfig.galaxyCount", normalized.galaxyCount, 1);
	assertInteger("generationConfig.systemsPerSector", normalized.systemsPerSector, 1);
	assertInteger("generationConfig.minPlanetsPerSystem", normalized.minPlanetsPerSystem, 1);
	assertInteger(
...
export function resolveGenerationTargets(
	...
	const targets = {
		...
		minUnclaimedColonizablePlanets:
			overrides.minUnclaimedColonizablePlanets ?? baseConfig.minUnclaimedColonizablePlanets,
	...
	assertInteger("minUnclaimedColonizablePlanets", targets.minUnclaimedColonizablePlanets, 0);
