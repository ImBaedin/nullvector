...
async function countColoniesByUniverse(
  ctx: MutationCtx,
  universeId: Id<"universes">
) {
...
async function countCoreSectorsByUniverse(
  ctx: MutationCtx,
  universeId: Id<"universes">
) {
  return await countByQuery((cursor) =>
...
async function countColonizablePlanetsByUniverse(
  ctx: MutationCtx,
  universeId: Id<"universes">
) {
  let cursor: string | null = null;
...
function hydrateGenerationState(args: {
  ...
  return {
    ...
    nextCoreSectorIndexByGalaxy,
    nextGalaxyCursor:
      galaxyCount > 0 ? baseState.nextGalaxyCursor % galaxyCount : 0,
...
export async function ensureCoreCapacityPipeline(
  ...
  for (let galaxyIndex = 0; galaxyIndex < normalizedConfig.galaxyCount; galaxyIndex += 1) {
    ...
    generationState.nextCoreSectorIndexByGalaxy[galaxyIndex] =
      await getNextSectorIndexForGalaxy(ctx, universe._id, galaxyIndex);
  }
  ...
  while (
    ...
    const galaxyRef = ensuredGalaxies.refs.find(
      (entry) => entry.galaxyIndex === galaxyIndex
    );
    if (!galaxyRef) {
      throw new ConvexError(`Missing galaxy reference for index ${galaxyIndex}`);
    }

    const startingSectorIndex =
      generationState.nextCoreSectorIndexByGalaxy[galaxyIndex] ?? 0;
  ...
  if (!params.dryRun) {
    await ctx.db.patch(universe._id, {
      ...
      generationConfig: {
        ...
        minCoreSectors:
          universe.generationConfig.minCoreSectors ??
          normalizedConfig.minCoreSectors,
        ...
        maxSectorsPerRun:
          universe.generationConfig.maxSectorsPerRun ??
          normalizedConfig.maxSectorsPerRun,
      },
      generationState,
      updatedAt: now,
    });
