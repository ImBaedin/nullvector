...
function assertGenerationToken(token: string) {
	...
	if (!configuredToken) {
		throw new ConvexError("UNIVERSE_GEN_TOKEN is not configured on this Convex deployment");
	}
...
async function resolveUniverse(ctx: MutationCtx, universeSlug?: string) {
	if (universeSlug) {
...
export const ensureCoreCapacity = mutation({
	...
	handler: async (ctx, args) => {
		...
		try {
			...
			const message = error instanceof Error ? error.message : "Universe generation failed";
