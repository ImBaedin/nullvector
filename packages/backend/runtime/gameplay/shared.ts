...
export type ProductionBuildingKey = "alloyMineLevel" | "crystalMineLevel" | "fuelRefineryLevel";
...
async function loadColonyState(args: { colony: Doc<"colonies">; ctx: QueryCtx | MutationCtx }) {
...
async function loadPlanetState(args: { planet: Doc<"planets">; ctx: QueryCtx | MutationCtx }) {
...
function getBuildingLaneCapacity(colony: Pick<ColonyState, "buildings">) {
	return (
		BUILDING_LANE_BASE_CAPACITY + Math.floor(Math.max(0, colony.buildings.roboticsHubLevel) / 2)
	);
}

function facilityLevelFromColony(colony: Pick<ColonyState, "buildings">, facilityKey: FacilityKey) {
...
function setFacilityLevelOnBuildings(args: {
	...
	if (args.facilityKey === ROBOTICS_HUB_FACILITY_KEY) {
		args.buildings.roboticsHubLevel = Math.max(args.level, args.buildings.roboticsHubLevel);
		return;
	}
...
function isStorageBuildingKey(buildingKey: BuildingKey): buildingKey is StorageBuildingKey {
	return (
...
function resolveDisplayName(authUser: { name?: string | null; email?: string | null }) {
	...
	const [activeRows, queuedRows] = await Promise.all([
		args.ctx.db
			...
			.withIndex("by_col_lane_st", (q) =>
				q.eq("colonyId", args.colonyId).eq("lane", args.lane).eq("status", "active"),
			)
			.collect(),
		args.ctx.db
			.query("colonyQueueItems")
			.withIndex("by_col_lane_st", (q) =>
				q.eq("colonyId", args.colonyId).eq("lane", args.lane).eq("status", "queued"),
			)
			.collect(),
	]);
...
async function settleShipyardQueue(args: {
	...
	const queuePatchById = new Map<Id<"colonyQueueItems">, Partial<Doc<"colonyQueueItems">>>();
	let activeQueue =
		queueRows.find((row) => row.status === "active" && isShipBuildQueueItem(row)) ?? null;
	const queued = queueRows.filter((row) => row.status === "queued" && isShipBuildQueueItem(row));
...
async function listOpenColonyQueueItems(args: {
	...
	const [activeRows, queuedRows] = await Promise.all([
		args.ctx.db
			.query("colonyQueueItems")
			.withIndex("by_col_st_time", (q) => q.eq("colonyId", args.colonyId).eq("status", "active"))
			.collect(),
		args.ctx.db
			.query("colonyQueueItems")
			.withIndex("by_col_st_time", (q) => q.eq("colonyId", args.colonyId).eq("status", "queued"))
			.collect(),
	]);
...
function toQueueViewItem(args: { item: Doc<"colonyQueueItems"> & QueuePayloadState; now: number }) {
	const { item, now } = args;
...
function buildHudResources(args: {
	...
	return [
		...
		},
		{
			...
			deltaPerMinuteAmount:
				crystalOverflowUnits > 0 ? 0 : Math.max(0, Math.floor(rates.resources.crystal)),
			...
			storagePercent: crystalCap <= 0 ? 0 : Math.min(100, (crystalUnits / crystalCap) * 100),
...
async function listPlayerColonyPlanets(args: { colonies: Array<Doc<"colonies">>; ctx: QueryCtx }) {
...
async function getBuildingQueueStatusForColony(args: {
	...
	const status: ColonyQueueStatus = hasActive ? "Upgrading" : hasPending ? "Queued" : "Stable";
	return status;
}
...
const buildingCardValidator = v.object({
	...
	name: v.string(),
	group: v.union(v.literal("Production"), v.literal("Power"), v.literal("Storage")),
...
const facilityCardValidator = v.object({
	...
	name: v.string(),
	category: v.union(v.literal("infrastructure"), v.literal("research"), v.literal("military")),
	currentLevel: v.number(),
	maxLevel: v.number(),
