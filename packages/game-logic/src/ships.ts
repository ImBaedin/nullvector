...
export function normalizeShipCounts(
  shipCounts: Partial<ShipCounts> | undefined,
): ShipCounts {
  return {
...
export function getShipBuildDurationSeconds(args: {
  shipKey: ShipKey;
...
export function getFleetFuelCostForDistance(args: {
  ...
  for (const key of Object.keys(DEFAULT_SHIP_DEFINITIONS) as ShipKey[]) {
    total +=
      normalized[key] * DEFAULT_SHIP_DEFINITIONS[key].fuelPerDistance * distance;
  }
