...
const STYLES: LanguageStyle[] = [
	{
		...
		syllablesMid: ["a", "e", "i", "o", "u", "ax", "or", "ul", "en", "ir", "ex", "ok"],
	...
	},
	{
		...
		syllablesMid: ["a", "e", "i", "o", "u", "li", "na", "ra", "se", "va", "ri", "lo"],
...
function sectorCode(rng: () => number, sectorNum?: number, galaxyNum?: number): string {
...
function generateByType(rng: () => number, style: LanguageStyle, parsed: ParsedSeed): string {
...
export function generateSciFiName(seed: string): string {
	...
	const style = STYLES[Math.floor(styleRng() * STYLES.length)] ?? (STYLES[0] as LanguageStyle);
