...
function buildArmPrimitive(args: {
  ...
  const widthStart = maxRadius * prng.nextInRange(
    profile.minArmWidthStartScale,
    profile.maxArmWidthStartScale
  );
  const widthEnd = maxRadius * prng.nextInRange(
    profile.minArmWidthEndScale,
    profile.maxArmWidthEndScale
  );
  const heightStart = maxRadius * prng.nextInRange(
    profile.minArmHeightStartScale,
    profile.maxArmHeightStartScale
  );
  const heightEnd = maxRadius * prng.nextInRange(
    profile.minArmHeightEndScale,
    profile.maxArmHeightEndScale
  );
  ...
  const endDir = normalize2([
    end.center[0] - preEnd.center[0],
    end.center[1] - preEnd.center[1],
  ]);
...
export function generateGalaxyModel(options: GalaxyModelOptions): GeneratedGalaxyModel {
  ...
  const coreRadius = maxRadius * prng.nextInRange(profile.minCoreRadiusScale, profile.maxCoreRadiusScale);
  const coreHeight = maxRadius * prng.nextInRange(profile.minCoreHeightScale, profile.maxCoreHeightScale);
