...
| Branch                     | Tier 1 Gate | Tier 2 Gate (also requires Research Directorate level 10)                | Tier 3 Gate (also requires Research Directorate level 20)                                           |
| -------------------------- | ----------- | ------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------- |
| `appliedIndustry`          | Always      | Reach any resource production building level 20 and any storage level 12 | Reach a combined 100 levels across resource production and storage buildings on one colony          |
| `militarySystems`          | Always      | Complete 15 contracts and successfully defend against 15 raids           | Complete 40 contracts, win 5 rank-3+ contracts, and own 500 combat ships                            |
| `scientificInfrastructure` | Always      | Spend a total of 50 meta-matter                                          | Spend a total of 250 meta-matter, earn 25 rare meta-matter, and reach Research Directorate level 20 |
| `expansionLogistics`       | Always      | Found a colony in another system                                         | Found a colony in another sector                                                                    |
| `colonySpecialization`     | Always      | Found 2 colonies and reach any facility level 12                         | Found 4 colonies and reach combined facility level 60 on a single colony                            |
...
| 3    | `outerRight` square  | `planetaryScaleFabrication`  | 2      | `distributedRobotics`, `industrialNetworkEffects`                              | Facility upgrade duration -12% per level for `robotics_hub`, `logistics_nexus`, and `research_directorate`. Level 2 adds +1 max level to those facilities.                                                                       |
...
| 2    | `outerLeft` circle   | `parallelInquiry`          | 3      | `archiveCompression`                               | For each Research Directorate level above a node's requirement, research duration is reduced by an additional 2%, capped at 10% at level 1, 15% at level 2, and 20% at level 3.                                            |
...
| 3    | `outerLeft` square   | `federatedDatabanks`       | 1      | `parallelInquiry`, `peerReviewProtocols`           | Enables combined Research Directorate capacity for future tier gates and node requirements. This should move the current late-game combined-capacity idea earlier than tier 4.                                             |
| 3    | `innerLeft` square   | `directorateExchange`      | 2      | `federatedDatabanks`, `activeCommandWindows`       | Each owned colony with Research Directorate level 5+ reduces research duration by 3%, capped at 12% at level 1 and 21% at level 2.                                                                                         |
...
- Multi-colony research duration scaling: `directorateExchange`.
