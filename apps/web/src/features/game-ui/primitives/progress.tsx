...
export function NvProgress({ className, value, tone = "neutral" }: NvProgressProps) {
  ...
  return (
    <div className={cn("relative h-2.5 w-full overflow-hidden rounded-full bg-[rgba(255,255,255,0.08)]", className)}>
      <div
        className={cn("relative h-full overflow-hidden rounded-full nv-transition", TONE_CLASS[tone])}
        style={{ width: `${bounded}%` }}
      >
        <div className="nv-progress-stripes absolute inset-0 rounded-[inherit] opacity-65" />
