import type { AlertDatum } from "@/features/game-ui/contracts/navigation";

import { NvBadge, NvPanel, NvScrollArea } from "@/features/game-ui/primitives";
   ...
   ">
				<ShellRail title="Alert Queue">
					<div className="space-y-2">
						{alerts.map((alert) => (
							<NvPanel
								className={cn(alert.severity === "danger" ? "nv-alert-emphasis" : null)}
								density="compact"
								key={alert.id}
								tone={
							...
							>
								<div className="flex items-center justify-between gap-2">
									<p className="text-sm text-(--nv-text-primary)">{alert.message}</p>
									<NvBadge
										tone={
											alert.severity === "danger"
												? "danger"
					...
					{rightRail ?? (
						<div className="space-y-2">
							<NvPanel density="compact">
								<p className="text-xs text-(--nv-text-muted)">Mission Density</p>
								<p className="nv-display mt-1 text-2xl">07</p>
							</NvPanel>
							<NvPanel density="compact">
								<p className="text-xs text-(--nv-text-muted)">Dock Queue</p>
