	...
	function UpgradeButton(
		...
		useEffect(() => {
			...
			const spawnArrow = () => {
				...
				timerRef.current = window.setTimeout(spawnArrow, 110 + Math.random() * 240);
			};
		...
		return (
			<button
				{...props}
				className={`
      group/upgrade relative min-w-[220px] overflow-hidden rounded-md border
      border-amber-200/55
      bg-[linear-gradient(180deg,rgba(255,222,152,0.32),rgba(255,165,86,0.18))]
      px-4 py-2 text-amber-50
      shadow-[inset_0_1px_0_rgba(255,255,255,0.25),0_6px_18px_rgba(255,159,67,0.26)]
      transition duration-300
      hover:border-amber-100/80
      hover:bg-[linear-gradient(180deg,rgba(255,230,169,0.4),rgba(255,174,95,0.22))]
      hover:shadow-[inset_0_1px_0_rgba(255,255,255,0.4),0_12px_28px_rgba(255,159,67,0.4)]
      aria-disabled:cursor-not-allowed aria-disabled:border-rose-300/65
					...
					{showArrowParticles
						? arrows.map((arrow) => (
								...
								>
									<ArrowUp
										className="
            size-3 text-[rgba(255,247,226,0.94)]
            drop-shadow-[0_0_6px_rgba(255,214,140,0.55)]
          "
										strokeWidth={2.8}
									/>
								</motion.span>
