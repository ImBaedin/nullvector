...
export function QueuePanel(props: {
	...
	return (
		<div className={cn("rounded-2xl border border-white/12 bg-[linear-gradient(170deg,rgba(12,20,36,0.95),rgba(6,10,18,0.98))]", props.className)}>
					...
					<div className="space-y-3">
						...
						<div className="rounded-xl border border-emerald-300/20 bg-emerald-400/4 p-3">
							...
							{props.activeItem.totalLabel ? (
								<p className="mt-2 font-(family-name:--nv-font-mono) text-[10px] text-white/35">
									{props.activeItem.totalLabel}
								</p>
							) : null}
							<div className="mt-2.5 h-1.5 overflow-hidden rounded-full bg-white/8">
								<div
									...
									style={{ width: `${props.activeProgressPercent}%` }}
								/>
							</div>
							<div className="mt-1 flex items-center justify-between">
								<span className="font-(family-name:--nv-font-mono) text-[9px] text-white/25">
									{Math.round(props.activeProgressPercent)}%
								</span>
						...
						<div className="mt-2 space-y-1">
							{props.pendingItems.map((item, index) => (
								...
								>
									...
									<div className="text-right">
										{item.totalLabel ? (
											<p className="font-(family-name:--nv-font-mono) text-[10px] text-white/35">
												{item.totalLabel}
											</p>
										) : null}
										{item.remainingLabel ? (
