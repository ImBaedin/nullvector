...
function CrumbButton({
  ...
  return (
    <button
      className="rounded-full border border-white/20 bg-white/5 px-3 py-1 text-xs text-slate-100 transition hover:bg-white/12 disabled:opacity-50"
      disabled={!onClick}
      onClick={onClick}
      type="button"
    >
...
export function ExplorerBreadcrumbs({
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      <CrumbButton
        onClick={() => explorer.setUniverseLevel({ x: 0, y: 0, zoom: ZOOM.universe })}
      >
        Universe
      </CrumbButton>
