...
export function HoverPanel({ hover }: { hover: HoverPanelState | null }) {
  ...
  return (
    <div
      className="pointer-events-none fixed z-50 w-[260px] overflow-hidden rounded-xl border border-cyan-200/25 bg-[linear-gradient(165deg,rgba(10,22,40,0.95),rgba(8,14,28,0.94))] p-0 text-xs text-slate-100 shadow-[0_10px_34px_rgba(0,0,0,0.45),0_0_0_1px_rgba(103,232,249,0.08)] backdrop-blur-sm"
      style={{
    ...
    >
      <div className="h-1.5 w-full bg-[linear-gradient(90deg,rgba(34,211,238,0.75),rgba(125,211,252,0.22),transparent)]" />
      ...
      <div className="space-y-3 px-3 py-3">
        <div className="flex items-start justify-between gap-2">
          ...
          <span className="relative mt-0.5 inline-flex size-4 items-center justify-center">
            <span className="absolute inline-flex size-4 rounded-full bg-cyan-300/45 animate-ping" />
            <span className="absolute inline-flex size-4 rounded-full border border-cyan-200/65 bg-cyan-300/15" />
        ...
        {hover.colonyName && hover.colonyPlayerName ? (
          <div className="rounded-lg border border-emerald-300/20 bg-emerald-400/[0.07] px-2.5 py-2">
            <p className="text-[9px] uppercase tracking-[0.18em] text-emerald-200/95">
              Colony Signal
            </p>
