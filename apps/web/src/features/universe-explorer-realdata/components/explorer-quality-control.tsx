...
export function ExplorerQualityControl({
  ...
  return (
    <div className="rounded-md border border-white/10 bg-white/5 px-3 py-2 text-xs text-slate-200">
      <p className="text-[11px] uppercase tracking-[0.18em] text-slate-300">Render quality</p>
        ...
        {QUALITY_LABELS.map((entry) => {
          ...
          return (
            <button
              ...
              className={
                isActive
                  ? "rounded border border-cyan-300/70 bg-cyan-400/20 px-2 py-1 text-[11px] text-cyan-100"
                  : "rounded border border-white/15 bg-white/5 px-2 py-1 text-[11px] text-slate-300 hover:bg-white/10"
              }
              onClick={() => onQualityPresetChange(entry.value)}
              type="button"
            >
              {entry.label}
            </button>
