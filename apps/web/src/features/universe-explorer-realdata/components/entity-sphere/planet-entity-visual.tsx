...
function PlanetModelVisual({
  ...
  const tiltX = useMemo(
    () => (hashStringToUnit(`${seedKey}:tiltX`) - 0.5) * 0.28,
    [seedKey]
  );
  const tiltY = useMemo(
    () => (hashStringToUnit(`${seedKey}:tiltY`) - 0.5) * 0.22,
    [seedKey]
  );
  ...
  const colorSet = useMemo(() => {
    const accentA = baseColor.clone().offsetHSL(
    ...
    const accentB = baseColor.clone().offsetHSL(
    ...
    clone.traverse((object) => {
      ...
      if (centeredGeometry.boundingSphere) {
        normalizedRadius = Math.max(
          normalizedRadius,
          centeredGeometry.boundingSphere.radius
        );
      }
      ...
      for (
        ...
        faceVertexIndex < positionAttr.count;
        faceVertexIndex += 3
      ) {
        ...
        const bandSignal =
          Math.sin(
            lon * patternConfig.bandFreq +
              lat * patternConfig.bandWarp +
              patternConfig.bandPhase
          ) +
          Math.sin(
            lat * (patternConfig.bandFreq * 1.75) + patternConfig.bandPhase * 0.7
          ) *
            0.28;
        const swirlSignal =
          Math.sin(
            (cx * 0.16 + cz * 0.2) * patternConfig.swirlFreq +
              cy * 0.32 +
              patternConfig.swirlPhase
          ) * 0.55;
        const patchSignal = Math.sin(
          (cx * 0.36 + cy * 0.29 + cz * 0.33) * patternConfig.patchFreq +
            patternConfig.patchPhase
        );
        ...
        const microShift =
          (hashStringToUnit(`${seedKey}:${faceVertexIndex}:micro`) - 0.5) * 0.05;
