...
function getGalaxyModelPath(seedKey: string) {
	const modelIndex =
		Math.floor(hashStringToUnit(seedKey) * GALAXY_MODEL_COUNT) % GALAXY_MODEL_COUNT;
	return `${GALAXY_MODEL_BASE_PATH}/galaxy-${modelIndex.toString().padStart(3, "0")}.glb`;
}

for (let index = 0; index < GALAXY_MODEL_COUNT; index += 1) {
	const path = `${GALAXY_MODEL_BASE_PATH}/galaxy-${index.toString().padStart(3, "0")}.glb`;
	useGLTF.preload(path);
}

type GalaxyDiscVisualProps = EntityVisualHandlers & {
	radius: number;
	detailLevel: "full" | "compact";
	quality: EntityVisualProps["quality"];
	coreColor: string;
...
type GalaxyModelVisualProps = EntityVisualHandlers & {
	radius: number;
	modelPath: string;
	quality: EntityVisualProps["quality"];
...
function GalaxyDiscVisual({
	...
	const ringSegments = quality === "high" ? (detailLevel === "compact" ? 24 : 36) : 20;
	...
	return (
		<>
			...
			<mesh rotation={[0, 0, ringRotationRad]} renderOrder={19} raycast={() => {}}>
...
function GalaxyModelVisual({
	...
	useFrame((_, delta) => {
		...
		if (occlusionFogMaterialRef.current) {
			...
			occlusionFogMaterialRef.current.uniforms.uOpacity.value = occlusionOpacity;
		}
	...
	return (
		<group
			...
			rotation={[0, 0, 0]}
		>
			<group scale={[modelScale, modelScale, modelScale]} renderOrder={30}>
				<Clone object={gltf.scene} />
			</group>
			<mesh scale={tuning.glow.ringScale} renderOrder={8} raycast={() => {}} rotation={[0, 0, 0.2]}>
				<ringGeometry
					args={[
			...
			<mesh scale={tuning.fog.occlusion.meshScale} renderOrder={13} raycast={() => {}}>
				<sphereGeometry
					args={[
          ...
          `}
            ...
            void main() {
              ...
              float alpha = (centerMask * ${tuning.fog.occlusion.alphaCenterMul.toFixed(
								3,
							)} + edgeMask * ${tuning.fog.occlusion.alphaEdgeMul.toFixed(3)}) * verticalMask;
              alpha *= (${tuning.fog.occlusion.alphaNoiseBase.toFixed(
								3,
							)} + noiseField * ${tuning.fog.occlusion.alphaNoiseMul.toFixed(3)});
          ...
          `}
			...
			<mesh scale={tuning.fog.haze.meshScale} renderOrder={12} raycast={() => {}}>
				<circleGeometry
          ...
          `}
            ...
            void main() {
              ...
              float alpha = bodyMask * edgeFade * (${tuning.fog.haze.alphaNoiseBase.toFixed(
								3,
							)} + noiseField * ${tuning.fog.haze.alphaNoiseMul.toFixed(3)}) * uOpacity;
...
export function GalaxyEntityVisual(props: EntityVisualProps) {
	const { radius, seedKey, quality, isSelected, isHovered, detailLevel, ...handlers } = props;
	const preset = getEntityVisualPreset("galaxy", hashStringToUnit(seedKey));
