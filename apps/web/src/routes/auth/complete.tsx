...
function AuthCompleteRoute() {
	...
	}, [hasBetterAuthSession, isAuthenticated, isAuthLoading, isBetterAuthPending, retryNonce]);
	...
	useEffect(() => {
		...
		(async () => {
			...
			.catch((error) => {
				const message =
					error instanceof Error ? error.message : "Failed to initialize your colony session";
     ...
     "
				>
					...
					<div className="flex items-center justify-center gap-2">
						<button
							className="
         rounded-sm border border-white/25 bg-white/10 px-3 py-1.5 text-sm
         text-white
         hover:bg-white/15
       "
