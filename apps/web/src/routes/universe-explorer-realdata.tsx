...
function UniverseExplorerScene() {
  ...
  const sidebar = (
    <div className="space-y-4">
      <div>
        <p className="text-[11px] uppercase tracking-[0.24em] text-cyan-200/85">
          Real Data
        </p>
        ...
        <p className="mt-2 text-sm text-slate-300/85">
          Isometric orthographic exploration of generated entities with drill
          navigation.
        </p>
      </div>
      ...
      <div>
        <h2 className="mb-2 text-xs uppercase tracking-[0.16em] text-slate-300">
          Current Entities
        </h2>
        ...
        {data.isCurrentLevelLoading ? (
          ...
          <p className="text-sm text-slate-300/80">
            No entities found for this level.
          </p>
        ) : (
          <div className="space-y-1">
            {currentEntities.map((entity) => (
              <button
                key={entity.id}
                className="flex w-full items-center justify-between rounded border border-white/10 bg-white/5 px-2 py-1 text-left text-xs hover:bg-white/10"
                onMouseEnter={(event) =>
