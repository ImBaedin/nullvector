  ...
  return (
    <Popover.Root>
      <Popover.Trigger
        ...
        openOnHover
        render={
          <button className="rounded-full border border-amber-100/35 bg-amber-300/20 p-1 text-amber-100">
...
export function QueuePanel(props: {
  ...
  return (
    <section className={`rounded-xl border border-white/12 bg-black/25 p-3 ${className ?? ""}`}>
      ...
      {!active ? (
        <p className="mt-2 rounded-lg border border-white/10 bg-black/20 p-2 text-xs text-white/70">
          No active build.
        </p>
      ) : null}
      ...
      <div className="mt-3 flex snap-x snap-mandatory gap-3 overflow-x-auto pb-2">
        {railItems.length === 0 ? (
          <p className="rounded-lg border border-white/10 bg-black/20 p-2 text-xs text-white/65">
            No queued builds.
          </p>
        ) : null}
        {railItems.map((item, index) => {
          ...
          return (
            <article
              className={`relative min-w-[320px] snap-start rounded-2xl border p-4 text-xs ${
                item.isActive
                  ? "border-cyan-200/45 bg-[linear-gradient(155deg,rgba(68,200,255,0.2),rgba(6,14,24,0.94))]"
                  : "border-white/12 bg-[linear-gradient(155deg,rgba(18,24,36,0.88),rgba(6,10,16,0.92))]"
              }`}
              key={item.id}
            >
              <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_8%_10%,rgba(255,255,255,0.09),transparent_35%)]" />
              <div className="flex items-start justify-between gap-2">
                ...
                <button
                  className="inline-flex items-center gap-1 rounded-md border border-rose-200/45 bg-rose-300/15 px-2 py-0.5 text-[11px] font-medium text-rose-100"
                  ...
                  {item.isActive ? (
                    <>
                      <div className="grid grid-cols-3 gap-3">
                        ...
                        <MetricInline label="Left" tone="cyan" value={item.remaining.toLocaleString()} />
                        <MetricInline label="Time Left" tone="cyan" value={formatDuration(item.timeLeftSeconds)} />
                      </div>
                    </>
                  ) : (
                    <>
              ...
              {showCosts && ship ? (
                <div className="mt-2 flex flex-wrap gap-1.5">
                  ...
                  <CostPill amount={ship.cost.crystal * item.total} kind="crystal" label="Crystal" />
              ...
              {item.isActive ? (
                <div className="mt-3 border-t border-white/10 pt-2">
                  ...
                  <p className="mt-1 inline-flex items-center gap-1 text-[11px] text-cyan-100/85">
