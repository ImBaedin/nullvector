...
const ColonyStarMapPickerContext =
  createContext<ColonyStarMapPickerContextValue | null>(null);

export function ColonyStarMapPickerProvider(props: { children: React.ReactNode }) {
  const [pickerRequest, setPickerRequest] = useState<StarMapPickerRequest | null>(
    null,
  );
  const [selectedTarget, setSelectedTarget] =
    useState<StarMapFleetTargetSelection | null>(null);
...
export function useColonyStarMapPicker() {
  ...
  if (!context) {
    throw new Error(
      "useColonyStarMapPicker must be used within ColonyStarMapPickerProvider",
    );
  }
  return context;
}
