				...
				{props.items.map((item) => {
					...
					return (
						...
						>
							...
							>
								<span
									className={`inline-block size-2 shrink-0 rounded-full ${item.dotClassName}`}
								/>
								<span className="min-w-0 shrink-0 text-xs font-semibold">{item.summaryLabel}</span>
								<span
									className={`shrink-0 rounded-sm px-1.5 py-0.5 text-[9px] font-semibold uppercase ${item.kindBadgeClassName}`}
								>
									{item.kindLabel}
								</span>
								{item.relationLabel ? (
									<span
										className={`shrink-0 rounded-sm px-1.5 py-0.5 text-[9px] font-semibold uppercase ${item.relationBadgeClassName ?? "border border-white/10 bg-white/3 text-white/70"}`}
									>
										{item.relationLabel}
									</span>
								) : null}
								...
								<div className="mx-1 hidden h-1 min-w-[60px] flex-1 overflow-hidden rounded-full bg-white/8 sm:block">
									<div
										className={`h-full rounded-full ${item.progressBarClassName}`}
									...
									<div className="border-t border-white/6">
										<div className="flex items-start px-5 pt-5 pb-8">
											...
											<div className="relative z-0 -mx-2 mt-5 min-w-[40px] flex-1">
												...
												<div
													className={`absolute top-0 h-px ${item.transitLineClassName}`}
													style={
														isExpanded
															? {
																	width: `${item.progress}%`,
												...
												>
													<div
														className={`flex size-6 items-center justify-center rounded-full border-2 shadow-lg ${item.transitIconBorderClassName} ${item.transitIconFillClassName}`}
													>
														{item.transitIcon}
													</div>
...
function EndpointNode(props: { delayMs: number; endpoint: ActivityEndpoint; isExpanded: boolean }) {
	return (
		...
		>
			<div
				className={`mx-auto flex size-10 items-center justify-center rounded-full border ${props.endpoint.iconContainerClassName}`}
			>
				{props.endpoint.icon}
			</div>
