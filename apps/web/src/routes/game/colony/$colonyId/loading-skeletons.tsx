...
function Sweep({ className }: SweepProps) {
	return (
		<div
			className={cn(
				"nv-loading-sweep rounded-xl border border-white/10 bg-white/3",
				className,
			)}
		/>
	);
...
function ColonyPageSkeletonFrame({ children }: { children: ReactNode }) {
	return <div className="mx-auto w-full max-w-[1440px] px-4 pt-4 pb-12 text-white">{children}</div>;
}

function ColonyTwoColumnSkeleton(args: {
	left: ReactNode;
	right: ReactNode;
}) {
	return (
